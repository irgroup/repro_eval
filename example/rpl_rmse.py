...
def main():
    ...
    for run_name, info in runs_rpl.items():
        with open(info.get('path')) as run_file:
            ...
            info['rmse'] = rpl_eval.rmse(run_b_score=info['scores'])


    baseline_runs = ['rpl_wcr04_tf_1', 'rpl_wcr04_tf_2', 'rpl_wcr04_tf_3', 'rpl_wcr04_tf_4', 'rpl_wcr04_tf_5']
    advanced_runs = ['rpl_wcr0405_tf_1', 'rpl_wcr0405_tf_2', 'rpl_wcr0405_tf_3', 'rpl_wcr0405_tf_4', 'rpl_wcr0405_tf_5']
    cutoffs = ['5', '10', '15', '20', '30', '100', '200', '500', '1000']

    df_content = {}
    for run_name in baseline_runs:
        df_content[run_name] = [runs_rpl[run_name]['rmse']['baseline']['ndcg_cut_' + co] for co in cutoffs]

    df = pd.DataFrame(df_content, index=cutoffs)
    ax = df.plot.line(style='o-')
    ax.set_xlabel('Cut-off values')
    ax.set_ylabel('RMSE')
    ax.get_figure().savefig('data/plots/rpl_b_rmse.pdf', format='pdf', bbox_inches='tight')
    plt.show()
    ...
    ax.set_xlabel('Cut-off values')
    ax.set_ylabel('RMSE')
    ax.get_figure().savefig('data/plots/rpl_a_rmse.pdf', format='pdf', bbox_inches='tight')
    plt.show()
